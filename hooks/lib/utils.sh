...
print_search_reminder() {
    ...
    echo "\`\`\`bash"
    echo "trekker semantic-search \"<what you're looking for>\""
    echo "\`\`\`"
    echo ""
    echo "**Use similar to detect duplicates before creating:**"
    echo "\`\`\`bash"
    echo "trekker similar \"<task description>\""
    echo "\`\`\`"
    echo ""
...
print_trekker_priority() {
    ...
    echo "---"
    echo ""
    echo "## CRITICAL: Trekker is Primary"
    echo ""
    echo "**Trekker MUST be prioritized over TodoWrite:**"
    echo "1. Create tasks in Trekker FIRST, then mirror to TodoWrite"
    echo "2. Update Trekker FIRST, then update TodoWrite"
    echo "3. When states conflict, Trekker wins"
    echo "4. Use \`trekker semantic-search\` and \`trekker similar\` to gather context"
    echo ""
}
